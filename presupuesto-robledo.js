...
<div id="rt-rpag" class="rob-pane2">
  ...
  // ── MOUNT (enganche compatible con la app principal) ─────────────────────
  let _rendered = false;
  function renderInto(container){
    container.innerHTML = '<div style="text-align:center;padding:28px;color:#8892b0;font-family:sans-serif;font-size:12px">⏳ Cargando presupuesto Robledo desde Supabase...</div>';
    fetchGastos().then(function(gastos){
      container.innerHTML = buildHTML(gastos);
      setupTabs(container);
      _rendered = true;
    });
  }
  function injectPresupuestoTab(){
    ...
    var lastBtn = document.getElementById('otab-ldg-robledo');
    if (!lastBtn) return;
    injectStyles();
    var btn = document.createElement('button');
    btn.id = 'otab-pre-robledo'; btn.className = 'obra-tab';
    btn.setAttribute('onclick', "obraTab('robledo','pre')");
    btn.innerHTML = '💰 Presupuesto';
    lastBtn.parentElement.appendChild(btn);
    var vRobledo = document.getElementById('v-robledo');
    if (!vRobledo) return;
    var div = document.createElement('div');
    div.id = 'otc-pre-robledo'; div.style.cssText = 'display:none;padding:12px 0';
    vRobledo.appendChild(div);
    var _orig = window.obraTab;
    window.obraTab = function(obra, tab){
      if (_orig) _orig(obra, tab);
      var c  = document.getElementById('otc-pre-robledo');
      var pb = document.getElementById('otab-pre-robledo');
      var isPre = (obra === 'robledo' && tab === 'pre');
      if (c)  c.style.display = isPre ? 'block' : 'none';
      if (pb) pb.classList.toggle('on', isPre);
      if (isPre && c && !_rendered) renderInto(c);
    };
  }
  function waitAndInject(){
    if (document.getElementById('otab-ldg-robledo')) { injectPresupuestoTab(); return; }
    var observer = new MutationObserver(function(){
      if (document.getElementById('otab-ldg-robledo')) { observer.disconnect(); injectPresupuestoTab(); }
    });
